...
const CoursePage = () => {
    ...
    async function getData() {
        try {
            const { data } = await axios.get(`${BE_URL}/courses/${id}`);
            setData(data.data);
        } catch (error) {
    ...
    async function sendReviewData(event) {
        ...
        try {
            ...
            await axios.post(`${BE_URL}/courses/${data.id}/reviews`, sendData);
            ...
            <div className="container">
                ...
                {data ?
                    <ul className="list-group">{data.professors.map(
                        (professor) => {
                            return <li key={professor.id} className="list-group-item list-group-item-dark list-group-item-action col-5"
                                ...
                                <form onSubmit={sendReviewData}>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        <textarea className="form-control" id="message" name="message" onChange={(e) => { setMessage(e.target.value) }}
                                            minLength="2" value={message} required></textarea>
                                    </div>
